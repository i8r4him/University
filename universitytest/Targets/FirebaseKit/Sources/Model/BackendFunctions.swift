...
extension DB {
	...
	public func processTextWithAI(text: String, readResultOutLoud: Bool) async -> TextAnalysisResultType? {
		Analytics.capture(
			.info, id: "process_text_with_ai_called",
			longDescription:
				"Start text processing with following text: '\(text)'. Should read the result out loud: \(readResultOutLoud ? "true" : "false").",
			source: .aikit)
		do {
			let contentDict: [String: Any] = ["text": text, "readOutLoud": readResultOutLoud]

			let data = try await functions.httpsCallable("analyzeTextContents").call(contentDict)

			guard let processResultDataRaw = data.data as? [String: Any] else {
				Analytics.capture(
					.error, id: "process_text_with_ai",
					longDescription: "Data Mismatch: data is not an array.", source: .aikit)
				return nil
			}
			Analytics.capture(.success, id: "process_text_with_ai", source: .aikit)
			return try? TextAnalysisResultType(from: processResultDataRaw)
		} catch {
			Analytics.capture(
				.error, id: "process_text_with_ai",
				longDescription: "Error processing text with AI: \(error.localizedDescription)", source: .aikit)
			return nil
		}
	}
